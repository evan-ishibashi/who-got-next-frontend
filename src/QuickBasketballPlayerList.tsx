...
function QuickBasketballPlayerList({ players, addPlayer, removePlayer }: { players: Player[], addPlayer:(name:string,idx:number)=> void, removePlayer:Function }) {
    ...
    return (
      ...
      <div className={`grid-container grid-cols-1 justify-evenly grid-flow-col gap-2 md:grid  md:gap-2 md:${players.length <31 ? 'grid-cols-3' : 'grid-cols-4'} grid-rows-10 md:overflow-x-auto`}>
